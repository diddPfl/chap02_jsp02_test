...
%>
   <!-- useBean 액션태그
   		1. scope="request": request 영역에서 뭔가를 찾느다.
   ...
   <!-- jsp:setProperty 액션 태그
   		1. name이 member인 객체의 setter메소드에 값을 세팅한다.
   		2. member 객체가 갖고 있는 setter 메소드에다 이전 화면에서 받은 파라미터를 자동으로 포집해준다.
   		3. html 폼에서 전송된 값을 자동으로 자바빈 클래스의 속성에서 바인딩해준다.
   		4. 이전 화면에서 name 속성에 있는 이름과 자바빈즈 도메인 클래스의 속성이름이 동일해야 가능하다.
